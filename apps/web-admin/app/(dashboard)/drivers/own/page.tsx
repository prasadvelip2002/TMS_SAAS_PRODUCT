...
export default function OwnFleetDriversPage() {
  ...
  const loadDrivers = async () => {
    try {
      const data = await fetchApi("/Drivers?isOwnFleet=true");
      setDrivers(data);
    } catch (error) {
  ...
  return (
    <div className="max-w-[1600px] mx-auto pb-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-slate-900 tracking-tight">Own Fleet Drivers</h1>
          <p className="text-xs sm:text-sm font-medium text-slate-500 mt-1">Manage company payroll driver profiles, licenses, and availability.</p>
        </div>
        <button 
              ...
              <div key={d.id} onClick={() => handleEdit(d)} className="bg-white border border-slate-200 rounded-2xl p-6 hover:shadow-lg transition-all cursor-pointer group relative flex flex-col h-full overflow-hidden">
                ...
                <div className="flex-1 space-y-3 mb-6 mt-2">
                  ...
                  <div className="flex items-center text-sm text-slate-600">
                    ...
                    <span className="truncate">Own Fleet</span>
                  </div>
                </div>
        ...
        <div className="fixed inset-0 z-[100] flex justify-end">
          ...
          <div className="relative w-full sm:max-w-md bg-white h-full shadow-2xl flex flex-col transform transition-transform duration-300 ease-in-out border-l border-slate-200">
             ...
             <div className="flex-1 overflow-y-auto p-4 sm:p-6 touch-scroll custom-table-scroll">
               <form id="driverForm" onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
                  <div>
