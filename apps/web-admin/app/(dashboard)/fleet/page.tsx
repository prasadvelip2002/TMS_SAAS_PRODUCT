  ...
  return (
    <div className="max-w-[1600px] mx-auto pb-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-slate-900 tracking-tight">Fleet Vendor Management</h1>
          <p className="text-xs sm:text-sm font-medium text-slate-500 mt-1">Manage your transport partners, documentation, and compliance.</p>
        </div>
        <button 
        ...
        <div className="fixed inset-0 z-[100] flex justify-end">
          ...
          <div className="relative w-full sm:max-w-md bg-white h-full shadow-2xl flex flex-col transform transition-transform duration-300 ease-in-out border-l border-slate-200">
             ...
             <div className="px-5 sm:px-6 py-4 sm:py-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
               <div>
                 <h3 className="font-bold text-base sm:text-lg text-slate-900 tracking-tight">{formData.id > 0 ? "Edit Vendor" : "New Vendor"}</h3>
                 <p className="text-[12px] sm:text-[13px] font-medium text-slate-500 mt-0.5">{formData.id > 0 ? "Update details" : "Onboard a new transport partner"}</p>
               </div>
               <button 
                 onClick={() => setIsFormOpen(false)} 
             ...
             <div className="flex-1 overflow-y-auto p-4 sm:p-6 touch-scroll custom-table-scroll">
               <form id="vendorForm" onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
                  <div>
