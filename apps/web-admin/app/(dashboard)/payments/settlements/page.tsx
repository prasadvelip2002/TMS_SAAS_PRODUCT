  ...
  return (
    <div className="relative min-h-full flex flex-col space-y-4">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-2 shrink-0">
        <div>
          <h1 className="font-disp font-bold text-2xl sm:text-[28px] text-slate-900 tracking-tight leading-tight">Vendor Settlements</h1>
          <p className="text-xs sm:text-[14px] text-slate-500 mt-1 font-medium">Settle final balances for vendors after PODs are received.</p>
        </div>
        
        <div className="flex flex-wrap items-center gap-2.5 sm:gap-3">
          {/* Status Tabs */}
          <div className="flex bg-slate-100 p-1 rounded-[12px] shadow-inner border border-slate-200/60">
            <button 
              onClick={() => setFilter("Pending")}
              className={`px-3 sm:px-4 py-1.5 rounded-[8px] text-xs sm:text-[13px] font-bold transition-all ${filter === "Pending" ? "bg-white text-slate-800 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
            >
              Pending
            </button>
            <button 
              onClick={() => setFilter("Settled")}
              className={`px-3 sm:px-4 py-1.5 rounded-[8px] text-xs sm:text-[13px] font-bold transition-all ${filter === "Settled" ? "bg-white text-slate-800 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
            >
              Settled
            </button>
            <button 
              onClick={() => setFilter("All")}
              className={`px-3 sm:px-4 py-1.5 rounded-[8px] text-xs sm:text-[13px] font-bold transition-all ${filter === "All" ? "bg-white text-slate-800 shadow-sm" : "text-slate-500 hover:text-slate-700"}`}
            >
              All
            </button>
          </div>
