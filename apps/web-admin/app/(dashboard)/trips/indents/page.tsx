  ...
  return (
    <div className="max-w-[1600px] mx-auto pb-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-slate-900 tracking-tight">Indent Management</h1>
          <p className="text-xs sm:text-sm font-medium text-slate-500 mt-1">Manage and track customer requests and load requirements.</p>
        </div>
        <button 
      ...
      <div className="bg-white rounded-2xl shadow-sm ring-1 ring-slate-200/50 p-2 mb-6 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 px-2 sm:pr-2 justify-end shrink-0">
          <button 
            ...
            className={`flex items-center gap-2 px-3 sm:px-4 py-2 rounded-xl font-bold text-xs sm:text-sm transition-all ${viewMode === 'grid' ? 'bg-blue-50 text-blue-700 border border-blue-100' : 'text-slate-500 hover:bg-slate-50 border border-transparent'}`}
          >
            <Grid className="w-4 h-4" /> Grid
          </button>
          <button 
            onClick={() => setViewMode('table')}
            className={`flex items-center gap-2 px-3 sm:px-4 py-2 rounded-xl font-bold text-xs sm:text-sm transition-all ${viewMode === 'table' ? 'bg-blue-50 text-blue-700 border border-blue-100' : 'text-slate-500 hover:bg-slate-50 border border-transparent'}`}
          >
                ...
                <div className="relative pl-6 space-y-2.5">
                  ...
                  <div className="relative flex items-center gap-2 min-w-0">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="font-bold text-slate-800 text-[13.5px] truncate mt-0.5" title={ind.source}>
                        {ind.source}
                      </div>
                    </div>
                  </div>
                  ...
                  {ind.warehouseLocation && (
                    <div className="relative flex items-center gap-1.5 min-w-0 py-0.5">
                      ...
                      <span className="text-[10.5px] font-bold text-amber-800 bg-amber-50 border border-amber-200 px-2 py-0.5 rounded-md truncate">
                        Transit via {ind.warehouseLocation} (Hub)
                      </span>
                    </div>
                  )}
                  ...
                  <div className="relative flex items-center gap-2 min-w-0">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="font-bold text-slate-800 text-[13.5px] truncate mt-0.5" title={ind.destination}>
                        {ind.destination}
                      </div>
                    </div>
                  </div>
            ...
            {filteredIndents.map((ind) => (
              <tr key={ind.id} className="hover:bg-slate-50 transition-colors cursor-pointer group" onClick={() => handleEdit(ind)}>
                ...
                <Td>
                  <button 
                    onClick={(e) => { e.stopPropagation(); handleDelete(ind.id); }}
                    className="text-muted-text hover:text-red-500 text-[12px] font-medium opacity-80 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity"
                  >
                    Delete
                  </button>
                </Td>
        ...
        <div className="fixed inset-0 z-[100] flex justify-end">
          ...
          <div className="relative w-full sm:max-w-md bg-white h-full shadow-2xl flex flex-col transform transition-transform duration-300 ease-in-out border-l border-slate-200">
             ...
             <div className="px-5 sm:px-6 py-4 sm:py-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
               <div>
                 <h3 className="font-bold text-base sm:text-lg text-slate-900 tracking-tight">{formData.id > 0 ? "Edit Indent" : "New Indent"}</h3>
                 <p className="text-[12px] sm:text-[13px] font-medium text-slate-500 mt-0.5">{formData.id > 0 ? "Update indent details" : "Create a new indent record"}</p>
               </div>
               <button 
                 onClick={() => setIsFormOpen(false)} 
             ...
             <div className="flex-1 overflow-y-auto p-4 sm:p-6 touch-scroll custom-table-scroll">
               <form id="indentForm" onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
                  {/* Customer Selection */}
