...
export default function OwnFleetVehiclesPage() {
  ...
  const loadData = async () => {
    try {
      const vehData = await fetchApi("/Vehicles?isOwnFleet=true");
      setVehicles(vehData);
    } catch (error) {
  ...
  return (
    <div className="max-w-[1600px] mx-auto pb-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-slate-900 tracking-tight">Own Fleet Vehicles</h1>
          <p className="text-xs sm:text-sm font-medium text-slate-500 mt-1">Manage company-owned fleet inventory, capacity, and compliance.</p>
        </div>
        <button 
      ...
      <div className="bg-white rounded-2xl shadow-sm ring-1 ring-slate-200/50 p-2 mb-6 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3">
        <div className="flex items-center px-3 sm:px-4 gap-3 flex-1 min-w-0">
          ...
          <input 
            type="text" 
            placeholder="Search own vehicles by reg number, type, code, owner..." 
            value={searchQuery}
        ...
        <div className="flex items-center gap-2 px-2 sm:pr-2 justify-end shrink-0">
          <button onClick={() => setViewMode('grid')} className={`flex items-center gap-2 px-3 sm:px-4 py-2 rounded-xl text-xs sm:text-sm font-bold border transition-colors ${viewMode === 'grid' ? 'bg-blue-50 text-blue-700 border-blue-100' : 'text-slate-500 border-transparent hover:bg-slate-50'}`}>
            <Grid className="w-4 h-4" /> Grid
          </button>
          <button onClick={() => setViewMode('list')} className={`flex items-center gap-2 px-3 sm:px-4 py-2 rounded-xl text-xs sm:text-sm font-bold border transition-colors ${viewMode === 'list' ? 'bg-blue-50 text-blue-700 border-blue-100' : 'text-slate-500 border-transparent hover:bg-slate-50'}`}>
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-2xl shadow-sm ring-1 ring-slate-200/50 overflow-hidden">
          <ProtoTable headers={["CODE", "REG. NUMBER", "TYPE", "CAPACITY", "OWNER", "STATUS", "ACTIONS"]}>
            {filteredVehicles.map((v) => (
              <tr key={v.id} className="hover:bg-slate-50 transition-colors cursor-pointer group" onClick={() => handleEdit(v)}>
                ...
                <Td className="text-[12px]">{v.ownerName || "Own Fleet"}</Td>
                <Td>
                  {v.status === 'Active' ? <span className="px-2 py-1 bg-green-50 text-green-700 text-xs font-bold rounded">Active</span> : <span className="px-2 py-1 bg-slate-100 text-slate-700 text-xs font-bold rounded">{v.status || "Draft"}</span>}
                </Td>
                <Td>
                  <button 
          ...
          {filteredVehicles.map((v) => (
            <div key={v.id} onClick={() => handleEdit(v)} className="bg-white border border-slate-200 rounded-2xl p-6 hover:shadow-lg transition-all cursor-pointer group relative flex flex-col h-full overflow-hidden">
              ...
              <div className="flex-1 space-y-3 mb-6 mt-2">
                ...
                <div className="flex items-center text-sm text-slate-600">
                  <span className="w-[80px] text-xs font-bold text-slate-400 uppercase tracking-wider">Owner</span>
                  <span className="truncate">{v.ownerName || "Own Fleet"}</span>
                </div>
              </div>
              
              <div className="pt-4 border-t border-slate-100 flex items-center justify-between mt-auto">
                <div>
                  {v.status === 'Active' ? <span className="px-2.5 py-1 bg-green-50 text-green-700 text-[10px] uppercase tracking-wider font-bold rounded border border-green-200">Active</span> : <span className="px-2.5 py-1 bg-slate-100 text-slate-700 text-[10px] uppercase tracking-wider font-bold rounded border border-slate-200">{v.status || "Draft"}</span>}
                </div>
                <button 
                  onClick={(e) => { e.stopPropagation(); handleDelete(v.id); }}
      ...
      {isFormOpen && (
        <div className="fixed inset-0 z-[100] flex justify-end">
          ...
          {/* Slide-over Panel */}
          <div className="relative w-full sm:max-w-md bg-white h-full shadow-2xl flex flex-col transform transition-transform duration-300 ease-in-out border-l border-slate-200">
             {/* Form Header */}
             <div className="px-5 sm:px-6 py-4 sm:py-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
               <div>
                 <h3 className="font-bold text-base sm:text-lg text-slate-900 tracking-tight">{formData.id > 0 ? "Edit Vehicle" : "New Vehicle"}</h3>
                 <p className="text-[12px] sm:text-[13px] font-medium text-slate-500 mt-0.5">{formData.id > 0 ? "Update details" : "Add a new vehicle to fleet"}</p>
               </div>
               <button 
                 onClick={() => setIsFormOpen(false)} 
             ...
             <div className="flex-1 overflow-y-auto p-4 sm:p-6 touch-scroll custom-table-scroll">
               <form id="vehicleForm" onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
